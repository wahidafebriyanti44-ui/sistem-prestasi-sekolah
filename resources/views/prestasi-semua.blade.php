    ...
    <div class="container mb-5">
        ...
        <div class="filter-card" data-aos="fade-up" data-aos-duration="800">
            <form action="{{ route('publik.prestasi.semua') }}" method="GET" class="row g-3 align-items-end">
                <div class="col-md-3">
                    ...
                    <input type="text" name="q" class="form-control" placeholder="Ketik nama siswa atau lomba..." value="{{ request('q') }}">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-2">
                    ...
                    <select name="tingkat" class="form-select">
                        ...
                        <option value="kabupaten" {{ request('tingkat') == 'kabupaten' ? 'selected' : '' }}>🏙️ Kabupaten/Kota</option>
                        <option value="provinsi" {{ request('tingkat') == 'provinsi' ? 'selected' : '' }}>🗺️ Provinsi</option>
            ...
            @if(request('q') || request('school_id') || request('tahun') || request('tingkat'))
            <div class="mt-3 pt-2 border-top">
                <small class="text-muted me-2">Filter aktif:</small>
                @if(request('q'))
                    <span class="badge-info me-1"><i class="bi bi-search"></i> {{ request('q') }}</span>
                @endif
                @if(request('school_id') && $selectedSchool = $sekolahList->firstWhere('id', request('school_id')))
                    <span class="badge-info me-1"><i class="bi bi-building"></i> {{ $selectedSchool->name }}</span>
                @endif
                @if(request('tahun'))
                    <span class="badge-info me-1"><i class="bi bi-calendar3"></i> {{ request('tahun') }}</span>
                @endif
                @if(request('tingkat'))
                    <span class="badge-info me-1"><i class="bi bi-geo-alt"></i> {{ ucfirst(request('tingkat')) }}</span>
                @endif
                <a href="{{ route('publik.prestasi.semua') }}" class="text-danger ms-2 text-decoration-none small">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
            <div>
                <span class="badge bg-primary bg-opacity-10 text-primary p-2 rounded-pill">
                    <i class="bi bi-trophy-fill me-1"></i> Menampilkan {{ $prestasi->total() }} prestasi
                </span>
            </div>
            <div>
                <a href="{{ route('publik.prestasi.semua') }}" class="btn-reset btn btn-sm rounded-pill px-3">
        ...
        <div class="row g-4">
            @forelse($prestasi as $item)
                <div class="col-lg-3 col-md-4 col-sm-6" data-aos="fade-up" data-aos-duration="800" data-aos-delay="{{ min($loop->iteration * 50, 500) }}">
                    <div class="achievement-card">
                        <div class="achievement-img-wrapper">
                            @php
                                $sertifikatPath = null;
                                if($item->file_sertifikat) {
                                    if(file_exists(public_path('uploads/sertifikat/' . $item->file_sertifikat))) {
                                        $sertifikatPath = asset('uploads/sertifikat/' . $item->file_sertifikat);
                                    } elseif(file_exists(public_path('storage/sertifikat/' . $item->file_sertifikat))) {
                                        $sertifikatPath = asset('storage/sertifikat/' . $item->file_sertifikat);
                                    }
                                }
                            @endphp
                            
                            @if($sertifikatPath && in_array(pathinfo($item->file_sertifikat, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'webp']))
                                <img src="{{ $sertifikatPath }}" class="achievement-img" alt="{{ $item->nama_lomba }}">
                            @else
                            ...
                            <div class="student-photo-badge">
                                @if($item->siswa && $item->siswa->foto && file_exists(public_path('uploads/foto-siswa/' . $item->siswa->foto)))
                        ...
                        <div class="achievement-content">
                            ...
                            <div class="text-secondary small mb-2">
                                ...
                                {{ $item->siswa->school->name ?? $item->siswa->sekolah->name ?? 'Sekolah Tidak Ditemukan' }}
                            </div>
                            
                            <!-- Tampilkan tingkat lomba -->
                            @if($item->tingkat)
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <span class="achievement-rank 
                                    {{ in_array($item->peringkat, ['Emas', 'Gold', 'Juara 1', '1']) ? 'rank-gold' : '' }}
                                    {{ in_array($item->peringkat, ['Perak', 'Silver', 'Juara 2', '2']) ? 'rank-silver' : '' }}
                                    {{ in_array($item->peringkat, ['Perunggu', 'Bronze', 'Juara 3', '3']) ? 'rank-bronze' : '' }}">
            ...
            @empty
                <div class="col-12 py-5 text-center" data-aos="fade-up">
                    <i class="bi bi-inbox display-1 text-secondary opacity-50 mb-3"></i>
                    <h4 class="text-secondary">Tidak ada prestasi yang ditemukan</h4>
                    <p class="text-muted">Coba sesuaikan filter pencarian Anda atau reset filter.</p>
