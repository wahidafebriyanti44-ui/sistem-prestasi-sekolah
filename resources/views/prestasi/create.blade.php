...
<body>
    ...
    <script>
        ...
        // ========== INDOREGION - DYNAMIC KABUPATEN (FIXED) ==========
        ...
        if (provinsiSelect) {
            provinsiSelect.addEventListener('change', function() {
                ...
                // Gunakan URL full untuk menghindari error route
                const url = '/get-kabupaten?provinsi_id=' + provinsiId;
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        kabupatenSelect.innerHTML = '<option value="">Tidak ada kabupaten/kota</option>';
                    } else {
                ...
                .catch(error => {
                    ...
                    kabupatenSelect.innerHTML = '<option value="">Gagal memuat data, coba lagi</option>';
                    kabupatenSelect.disabled = false;
                });
            });
        }
        ...
        if (fileInput) {
            fileInput.addEventListener('change', function(e) {
                ...
                if (file) {
                    ...
                    if (fileNameSpan) fileNameSpan.textContent = file.name + ' (' + (file.size / 1024).toFixed(2) + ' KB)';
                    if (selectedFileDiv) selectedFileDiv.style.display = 'flex';
                    if (uploadText) uploadText.innerHTML = '<i class="fas fa-check-circle" style="color: #28a745;"></i> File dipilih';
                } else {
                    if (selectedFileDiv) selectedFileDiv.style.display = 'none';
                    if (uploadText) uploadText.innerHTML = 'Klik untuk upload file sertifikat (PDF/JPG/PNG)';
                }
            });
        }
