...
<head>
    ...
    <title>Edit Minat & Bakat - SIPRES</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-control, .form-select {
            width: 100%; padding: 12px 16px; border: 2px solid #E5E9F0;
        ...
        .required-note {
            font-size: 12px; color: var(--secondary); margin-top: 10px;
            background: var(--gray); padding: 8px 12px; border-radius: 8px;
        }
        ...
        .current-avatar { margin-top: 8px; display: flex; align-items: center; gap: 8px; padding: 8px; background: var(--gray); border-radius: 10px; }
        ...
        .btn-cancel { padding: 10px 20px; background: var(--gray); color: var(--dark); border: none; border-radius: 10px; font-weight: 600; cursor: pointer; }
    ...
    <div class="app">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <div class="sidebar-profile">
                <div class="profile-card">
                    <img src="{{ Auth::user()->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name ?? 'User').'&background=2A5C8A&color=fff' }}" class="profile-avatar" id="profileImage">
        ...
        <div class="main-content">
            ...
            <div class="alert alert-danger">
                ...
                Terdapat kesalahan pada input data. Silakan periksa kembali.
            </div>
            @endif
            ...
            <div class="card">
                <div class="card-header">
                    <h2><i class="fas fa-edit"></i> Form Edit Minat & Bakat</h2>
                </div>
                <div class="card-body">
                    <form action="{{ route('minat-bakat.update', $minatBakat) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <div class="form-grid">
                            <div class="form-group full-width">
                                <label for="nama_minat" class="form-label">
                                    <i class="fas fa-heart"></i> Nama Minat & Bakat <span class="required">*</span>
                                </label>
                                <input type="text" 
                                       id="nama_minat" 
                                       name="nama_minat" 
                                       value="{{ old('nama_minat', $minatBakat->nama_minat) }}" 
                                       class="form-control @error('nama_minat') is-invalid @enderror" 
                                       placeholder="Contoh: Sepak Bola, Melukis, Coding"
                                       required>
                                @error('nama_minat')
                            ...
                            <div class="form-group">
                                ...
                                <select id="kategori" 
                                    ...
                                    <option value="olahraga" {{ old('kategori', $minatBakat->kategori) == 'olahraga' ? 'selected' : '' }}>⚽ Olahraga</option>
                                    <option value="seni" {{ old('kategori', $minatBakat->kategori) == 'seni' ? 'selected' : '' }}>🎨 Seni</option>
                                    <option value="sains" {{ old('kategori', $minatBakat->kategori) == 'sains' ? 'selected' : '' }}>🔬 Sains</option>
                                    <option value="bahasa" {{ old('kategori', $minatBakat->kategori) == 'bahasa' ? 'selected' : '' }}>📚 Bahasa</option>
                                    <option value="teknologi" {{ old('kategori', $minatBakat->kategori) == 'teknologi' ? 'selected' : '' }}>💻 Teknologi</option>
                                    <option value="lainnya" {{ old('kategori', $minatBakat->kategori) == 'lainnya' ? 'selected' : '' }}>📌 Lainnya</option>
                                </select>
                                @error('kategori')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="form-group full-width">
                                ...
                                <textarea id="deskripsi" 
                                          ...
                                          rows="4"
                                          placeholder="Deskripsikan minat & bakat ini (opsional)">{{ old('deskripsi', $minatBakat->deskripsi) }}</textarea>
                                @error('deskripsi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text" style="font-size: 11px; color: var(--secondary); margin-top: 4px;">
                                    <i class="fas fa-info-circle"></i> Deskripsi dapat membantu siswa memahami lebih lanjut tentang minat & bakat ini.
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-actions">
                            <a href="{{ route('minat-bakat.index') }}" class="btn btn-secondary">
                                <i class="fas fa-times"></i> Batal
                            </a>
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Update Minat & Bakat
                            </button>
                        </div>
                    </form>
                </div>
