...
<head>
    ...
    <title>Data Siswa - SIPRES | Sistem Informasi Prestasi Siswa</title>
    ...
    <div class="container">
        ...
        <div class="main-content" id="mainContent">
            ...
            <div class="info-banner">
                ...
                <p>Mode Super Admin - Anda dapat melihat semua siswa dari seluruh sekolah, namun <strong>tidak dapat menambah, mengedit, atau menghapus</strong> data siswa.</p>
            </div>
            @endif
            ...
            <div class="action-bar">
                <h2>
                    ...
                    @if(Auth::user()->role == 'super_admin')
                        Daftar Siswa Semua Sekolah
                    @else
                        Daftar Siswa {{ Auth::user()->school->name ?? '' }}
                    @endif
                </h2>
            ...
            <div class="filter-section">
                <form method="GET" action="{{ route('siswa.index') }}" class="filter-form">
                    <div class="filter-group">
                        <label class="filter-label"><i class="fas fa-search"></i> Cari Nama / NIS / NISN</label>
                        <div class="search-wrapper">
                            <i class="fas fa-search search-icon"></i>
                            <input type="text" name="search" class="search-input" placeholder="Ketik nama, NIS, atau NISN..." value="{{ request('search') }}">
                        </div>
                    </div>
                    ...
                    <div class="filter-group">
                        <label class="filter-label"><i class="fas fa-school"></i> Filter Kelas</label>
                    ...
                    <div class="filter-actions">
                        ...
                        <a href="{{ route('siswa.index') }}" class="btn-filter">
            ...
            <!-- TAMPILAN UNTUK SUPER ADMIN (GROUP BY SEKOLAH) - MENGGUNAKAN GRID CARD -->
            @if(Auth::user()->role == 'super_admin')
                @php
                    // Group siswa berdasarkan sekolah
                    $groupedSiswa = $siswa->groupBy(function($item) {
                        return $item->school_id ?? ($item->kelas->school_id ?? null);
                    });
                @endphp

                @forelse($groupedSiswa as $schoolId => $siswaSekolah)
                    @php
                    ...
                    <div class="school-header">
                        ...
                        <span class="school-badge">
                            <i class="fas fa-users"></i> Total: {{ $siswaSekolah->count() }} Siswa
                        </span>
                    </div>
                    
                    <div class="card-grid">
                        @foreach($siswaSekolah as $s)
                        <div class="card">
                            <div class="card-header">
                                <div class="card-avatar">
                                    @if($s->foto && file_exists(public_path('uploads/foto-siswa/' . $s->foto)))
                                        <img src="{{ asset('uploads/foto-siswa/' . $s->foto) }}" alt="{{ $s->nama_lengkap }}">
                                    @else
                                        {{ strtoupper(substr($s->nama_lengkap, 0, 1)) }}
                                    @endif
                                </div>
                                <span class="card-badge">
                                    <i class="fas fa-id-card"></i> NIS: {{ $s->nis }}
                                </span>
                            </div>
                            
                            <div class="card-body">
                                <h3 class="card-title">{{ $s->nama_lengkap }}</h3>
                                
                                <div class="card-info">
                                    <div class="info-row">
                                        <i class="fas fa-school"></i>
                                        <span>Sekolah: <strong class="school-name">{{ $school->name ?? '-' }}</strong></span>
                                    </div>
                                    <div class="info-row">
                                        <i class="fas fa-chalkboard"></i>
                                        <span>Kelas: <strong>{{ $s->kelas->nama_kelas ?? '-' }}</strong></span>
                                    </div>
                                    <div class="info-row">
                                        <i class="fas {{ $s->jenis_kelamin == 'L' ? 'fa-mars' : 'fa-venus' }}"></i>
                                        <span>Jenis Kelamin: <strong>{{ $s->jenis_kelamin == 'L' ? 'Laki-laki' : ($s->jenis_kelamin == 'P' ? 'Perempuan' : '-') }}</strong></span>
                                    </div>
                                    <div class="info-row">
                                        <i class="fas fa-phone"></i>
                                        <span>No. HP: <strong>{{ $s->no_hp ?? '-' }}</strong></span>
                                    </div>
                                    @if($s->nisn)
                                    <div class="info-row">
                            ...
                            <!-- SUPER ADMIN HANYA BISA LIHAT DETAIL -->
                            <div class="card-footer">
                                <a href="{{ route('siswa.show', $s->id) }}" class="btn-action btn-view">
                ...
                @empty
                    <div class="empty-state">
                        <i class="fas fa-users"></i>
                        <p>Belum ada data siswa di semua sekolah</p>
                    </div>
                @endforelse

            @else
                <!-- TAMPILAN UNTUK ADMIN SEKOLAH (GRID CARD DENGAN EDIT & HAPUS) -->
                <div class="card-grid">
                    @forelse($siswa as $s)
                    <div class="card">
                        <div class="card-header">
                        ...
                        <div class="card-footer">
                            ...
                            <form action="{{ route('siswa.destroy', $s->id) }}" method="POST" style="flex: 1;" onsubmit="return confirm('Yakin ingin menghapus siswa {{ $s->nama_lengkap }}?')">
                                @csrf
                                @method('DELETE')
            ...
            <div class="pagination-container">
                ...
                <ul class="pagination">
                    ...
                    @else
                        <li class="page-item"><a class="page-link" href="{{ $siswa->previousPageUrl() }}"><i class="fas fa-chevron-left"></i></a></li>
                    @endif
                    ...
                    @if($siswa->hasMorePages())
                        <li class="page-item"><a class="page-link" href="{{ $siswa->nextPageUrl() }}"><i class="fas fa-chevron-right"></i></a></li>
                    @else
